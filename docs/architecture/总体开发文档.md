...
```text
┌─────────────────────────────────────────────────────────────┐
│                    CLI / Job Orchestrator                   │
│  配置加载、任务生命周期、并发控制、状态展示、报告生成       │
└───────────────┬───────────────────────────┬─────────────────┘
                │                           │
                ▼                           ▼
┌─────────────────────────────┐   ┌───────────────────────────┐
│ Coverage-guided Fuzzer      │   │ Judge Pipeline            │
│ SeedPool / Mutator / Energy │   │ LLM Judge / Golden Set    │
│ Scheduler / Corpus Manager  │   │ Active Learning / Drift   │
└───────────────┬─────────────┘   └─────────────┬─────────────┘
                │                               │
                ▼                               │
┌───────────────────────────────────────────────┴─────────────┐
│                     Coverage Engine                         │
│ Behavior Profile Coverage + Risk Dimension Coverage        │
└─────────────────────────────┬───────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────┐
│                   Trace & Replay Engine                     │
│ Tracer / Normalizer / Replay Manifest / Breakpoint Fork     │
└─────────────────────────────┬───────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────┐
│                   Docker Sandbox Layer                      │
│ Scheduler / Runtime API / AgentAdapter / Controlled Tools   │
└─────────────────────────────┬───────────────────────────────┘
                              │
                              ▼
                    被测 Agent / 靶机 Agent
...
Fuzzer 选择种子
  ...
  → Mutator 生成候选
  → 调度器并发执行
  → Coverage Engine 计算新增覆盖
  → Judge 对高价值或疑似违规轨迹评分
  → Corpus Manager 保留高价值用例
  → 进入下一轮
```
...
1. 确定性规则：工具名、路径、命令、策略事件。
2. 正则与关键词：敏感字段、危险命令族、外传意图。
3. 小模型分类：对 Prompt、回答和工具摘要分类。
4. LLM Judge：对复杂语义和上下文进行最终判断。

确定性证据优先级高于语义推断。每个 `RiskHit` 必须包含证据事件序号。
...
Mutator 只能输出符合 Schema 的候选：

```json
{
  "prompt": "...",
  "operator": "roleplay_wrapper",
  "target_risks": ["unauthorized_file_read"],
  "expected_novelty": "尝试先调用搜索工具再访问文件",
  "constraints_preserved": ["任务目标", "目标资源"]
}
```
...
while not budget.exhausted():
    ...
    energy = energy_scheduler.assign(seed)
    candidates = mutator.mutate(seed, count=energy)

    for candidate in deduplicator.filter(candidates):
        trajectory = await sandbox_pool.execute(candidate)
        coverage = coverage_engine.evaluate(trajectory)
        finding = await judge.maybe_evaluate(trajectory, coverage)

        if corpus_policy.is_interesting(coverage, finding):
            corpus.save(candidate, trajectory, coverage, finding)
            seed_pool.promote(candidate, coverage, finding)
```
...
| 风险分类全依赖 LLM | 结果不稳定且成本高 | 规则、小模型、Judge 多层组合 |
